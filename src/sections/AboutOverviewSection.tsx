...
function renderTitle(title: string, highlight?: string) {
  ...
  return (
    <>
      {renderLines(parts[0])}
      <span className="italic font-light text-gradient-ayuta inline-block">
        {renderLines(highlight)}
      </span>
      {renderLines(parts[1])}
    </>
  );
    ...
    <section ref={containerRef} className="py-40 px-6 bg-black overflow-hidden">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        <div ref={textRef} className="order-2 lg:order-1">
          ...
          <h3 className="text-5xl md:text-7xl font-display font-medium tracking-tight mb-10 text-white leading-[1.1] whitespace-pre-line">
